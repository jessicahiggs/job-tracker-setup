...
Read `references/gmail_queries.md` before running any searches — it has the exact query patterns for each signal type (new application, rejection, interview, offer, active dialogue, referral) along with the reasoning behind each one, so you're not reinventing search strings from scratch. Read `references/status_rules.md` for the status/notes rules that keep the tracker trustworthy over many runs.

> **Hard-won lessons.** This skill has since been stress-tested against a real, active job search where the naive version made real mistakes. The most important fixes are called out in the reference docs and worth internalizing before you run it:
> - **Never trust one phrase to detect an application.** Confirmation wording varies wildly ("thank you for your application", "we have received your application", "we are lucky that you applied"). Match on a wide phrase list *and* a sender heuristic, and also sweep the job-search label directly. (See `gmail_queries.md` §1.)
> - **Read every email's body for the exact role — don't infer from the subject or from how Gmail groups messages.** One conversation can contain confirmations for two *different* roles at the same company. Companies do not send duplicate confirmations for one application, so two confirmations with different role text are two different applications. Never merge them. (See `status_rules.md` → Row matching.)
> - **Convert dates to the user's local timezone.** Gmail timestamps may be UTC; an email just after midnight UTC is usually the previous evening locally. Getting this wrong makes "today's applications" land on the wrong day.
> - **A tracker only sees the account it's connected to.** Mail the user sent or received from a *different* address won't appear — say so rather than reporting a gap as if it's complete.
...
1. **Which Gmail account/connector** should be scanned? If more than one Gmail MCP connector is available, ask which one. Remind them the tracker only sees this account — if they apply from a second address, those won't be captured.
...
4. **Status categories** — the default set is `Applied`, `Warm`, `Interviewing`, `Offer`, `Rejected` (see `status_rules.md` for what each means and how they progress). Confirm this works, or learn their preferred set. Keep it to a small fixed set; more than 5 statuses makes the "never regress" rule hard to reason about. `Warm` is optional but genuinely useful — it captures "this specific role didn't proceed, but a recruiter is keeping me warm / referring me elsewhere," a state that otherwise gets mislabeled as either Rejected or Interviewing.
...
```json
{"lastSync": "<today's date, user's local timezone>", "rows": [{"company": "", "role": "", "status": "", "date": "", "notes": ""}]}
```

## Step 3 — Scan and build

Run the searches and status/notes logic from `references/gmail_queries.md` and `references/status_rules.md` against the window since `lastSync`. This is identical logic whether it's the first run or the hundredth — the only thing that changes is the date window. Use the user's local calendar date everywhere (convert Gmail's UTC timestamps).
...
If the user wanted a recurring sync (Step 1.5), invoke the `schedule` skill to set up a scheduled task that repeats Steps 3–4 (scan since last sync, rebuild artifact) using the same state file and artifact id. The scheduled prompt must be fully self-contained (a fresh run has no memory of this conversation): embed the account, label id, artifact id, state-file path, status set, and the query/status rules. Otherwise, tell the user how to re-trigger a sync on demand (e.g. "just ask me to sync your tracker again").

## Guardrails

- **Read each application email individually.** Extract the exact role from the body, not the subject or how messages are grouped. A single conversation can hold confirmations for two different roles — keep them as separate rows. Companies don't send duplicate confirmations, so two confirmations with different role text = two applications.
...
- Never regress a status once it's reached `Rejected` or `Offer` from real evidence. (A genuine *re-application* to a previously-rejected role is the one documented exception — see `status_rules.md`.)
- `Warm` is sticky: don't bump it to `Interviewing` off an old email; only a genuinely new interview/offer email for that exact role changes it.
- Never delete existing note text — always append with a `[MM/DD]` tag.
- Be transparent about which account is connected, where state is saved, and any limitations (e.g., "this won't survive past this session unless you connect a folder"; "mail from your other email address won't show up here").

## Known limitations

- **Single account only.** The tracker sees exactly the one Gmail account it's connected to. Applications sent from, or replies received at, a different address are invisible to it. Tell the user this explicitly.
...
- **Confirmation wording is endlessly varied.** Even in English, ATS/careers systems phrase "we got your application" dozens of ways. §1 of `gmail_queries.md` casts a wide net (phrases + sender heuristic + label sweep), but a brand-new sender pattern can still slip through — when a user says an application is missing, believe them and widen the net rather than defending the matcher.
